...
def get_values_from_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to real images to be tested",
    )
    parser.add_argument(
        "--test-type",
        "-t",
        type=str,
        default="real",
        help="choice of evaluation type (sewfactory, deepfashion, real)",
    )
    parser.add_argument(
        "--save-root",
        "-o",
        type=str,
        default="",
        help="output path for the predicted sewing pattern",
    )
...
class DeepFashionDataset(torch.utils.data.Dataset):
    def __init__(self, deep_fashion_root, resolution=1024):
        ...
        self._ref_image_fnames = [
            os.path.join(root, fname)
            for root, _dirs, files in os.walk(os.path.join(self._target_path))
            for fname in files
        ]
        self._ref_image_fnames = [
            (i, self.get_segm_path(i)) for i in self._ref_image_fnames
        ]
        self._ref_image_fnames_has_mask = [
            i for i in self._ref_image_fnames if i[1] is not None
        ]
        self._ref_image_fnames_has_mask_woman = [
            i for i in self._ref_image_fnames_has_mask if "WOMEN" in i[0]
        ]
        print(
            len(self._ref_image_fnames),
            len(self._ref_image_fnames_has_mask),
            len(self._ref_image_fnames_has_mask_woman),
        )

        self._img_transform = T.Compose([T.Resize((384, 384)), T.ToTensor()])

    def get_segm_path(self, path):
        _, path = os.path.split(path)
        img_id = os.path.splitext(path)[0]
        segm_path = os.path.join(self._target_path, "segm", "{}_segm.png").format(
            img_id
        )
    ...
    def _get_source_appearance(self, idx):
        if len(self._ref_image_fnames_has_mask_woman) == 0:
            ...
            ref_img_mask = np.array(
                Image.fromarray((np.ones(ref_img.shape[:2]) * 255).astype("uint8"))
            )
        else:
        ...
        # remove background
        ref_img = (
            ref_img * (ref_img_mask > 0)[:, :, np.newaxis]
            + (ref_img_mask == 0)[:, :, np.newaxis] * 255
        )
...
if __name__ == "__main__":
    ...
    shape_experiment = ExperimentWrappper(
        config, system_info["wandb_username"]
    )  # finished experiment
    ...
    if args.test_type != "sewfactory":
        ...
        shape_dataset, shape_datawrapper = shape_experiment.load_detr_dataset(
            (
                Path(system_info["datasets_path"])
                if args.unseen
                else system_info["datasets_path"]
            ),  # assuming dataset root structure
            {
                "feature_caching": False,
                "gt_caching": False,
    ...
    if args.test_type == "real":
        fns = [
            ...
            for fn in os.listdir(args.data_root)
            if is_img_file(fn)
        ]
        for idx, fn in enumerate(fns):
            ...
            _, _, prediction_img = shape_dataset.save_prediction_single(
                output, dataname=dataname, save_to=args.save_root, return_stitches=False
            )
        ...
        for idx in range(len(deepfashion_dataset)):
            ...
            _, _, prediction_img = shape_dataset.save_prediction_single(
                output, dataname=dataname, save_to=pred_save_root, return_stitches=False
            )
            shutil.copy2(dataname_path, os.path.dirname(prediction_img))
            print(f"end of #{idx}, {dataname_path} ...")
    elif args.test_type == "sewfactory":
        final_metrics, score_dict, _ = eval_detr_metrics(
            shape_model, criterion, shape_datawrapper, 0, "test"
        )

    else:
        raise NotImplementedError(
            "Not implemented test type: {}".format(args.test_type)
        )
