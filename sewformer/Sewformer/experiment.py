    ...
    * for finished experiments, if w&b run info is specified, it has priority over the localy stored information
    ...
    def init_run(self, config={}):
        ...
        wb.init(
            ...
            config=config,
            resume="allow",
            id=self.run_id,  # Resuming functionality
        ...
        if not self.wandb_username:
            ...
            print(
                f"{self.__class__.__name__}::WARNING::Running wandb in Anonymous Mode. Your temporary account is: {self.wandb_username}"
            )
    ...
    def data_info(self):
        ...
        try:
            ...
            data_config["max_datapoints_per_type"] = (
                None  # avoid slicing for correct loading of split on any machine
            )
        except (
            ValueError,
            RuntimeError,
        ) as e:  # if file not found, training will just proceed with generated split

            print(
                f"{self.__class__.__name__}::Warning::Skipping loading split file from cloud.."
            )
        ...
        try:
            ...
            print(
                f"{self.__class__.__name__}::Warning::Skipping loading panel classes file from cloud.."
            )
        ...
        try:
            ...
            ValueError,
            RuntimeError,
        ) as e:  # if file not found, training will just proceed with given setup
            print(
                f"{self.__class__.__name__}::Warning::Skipping loading parameter filter file from cloud.."
            )

        # This part might be missing from the configs loaded from W&B runs
        if (
            "unseen_data_folders" not in data_config
            and "dataset" in self.in_config
            and "unseen_data_folders" in self.in_config["dataset"]
        ):
            data_config["unseen_data_folders"] = self.in_config["dataset"][
                "unseen_data_folders"
            ]
    ...
    def last_best_validation_loss(self):
        ...
        run = self._run_object()
        return (
            run.summary["best_valid_loss"] if "best_valid_loss" in run.summary else None
        )
    ...
    def add_statistic(self, tag, info, log=""):
        ...
        if not self.run_id:
            print(
                f"{self.__class__.__name__}::Warning::Experiment not connected to the cloud. Statistic {tag} not synced"
            )
            return
    ...
    def add_config(self, tag, info):
        ...
        if self.initialized:
            ...
            raise RuntimeError(
                "ExperimentWrappper:Error:Cannot add config to finished run"
            )
    ...
    def add_artifact(self, path, name, type):
        ...
        if not self.run_id:
            print(
                f"{self.__class__.__name__}::Warning::Experiment not connected to the cloud. Artifact {name} not synced"
            )
            return
        ...
        if not self.initialized:
            ...
            print(
                "Experiment::Reactivating wandb run to upload an artifact {}!".format(
                    name
                )
            )
    ...
    def is_finished(self):
        if not self.run_id:
            print(
                f"{self.__class__.__name__}::Warning::Requested status of run not connected to wandb"
            )
            return True
        run = self._run_object()
    ...
    def load_dataset(
        self, data_root, eval_config={}, unseen=False, batch_size=5, load_all=False
    ):
        ...
        if unseen:
            ...
            data_config.update(
                data_folders=data_config["unseen_data_folders"]
            )  # use the unseen folders list
        ...
        if "wrapper" in data_config and data_config["wrapper"] is not None:
            ...
            datawrapper = datawrapper_class(
                dataset, known_split=split, batch_size=batch_size
            )
        else:
    ...
    def load_detr_dataset(
        ...
        split, _, data_config = (
            self.data_info()
        )  # note that run is not initialized -- we use info from finished run
        if unseen:
            load_all = True  # load data as a whole without splitting
            data_config.update(
                data_folders=data_config["unseen_data_folders"]
    ...
    def recover_detr_model(self, data_config=None):
        ...
        model, criterion = models.build_former(
            yaml.safe_load(open(self.in_config["experiment"]["local_path"], "r"))
        )
    ...
    def prediction(
        ...
        if nick:
            self.add_statistic(
                ...
                log="Prediction save path",
            )

        if art_name:
            art_name = (
                art_name
                if len(datawrapper.dataset.data_folders) > 1
                else datawrapper.dataset.data_folders[0]
            )
    ...
    def prediction_single(
        self, save_to, model, datawrapper, image, data_name="outside_single"
    ):
        panel_order, panel_idx, prediction_img = datawrapper.predict_single(
            model, image, data_name, save_to
        )
    ...
    def cloud_path(self):
        ...
        if not self.run_id:
            raise RuntimeError(
                "ExperimentWrappper:Error:Need to know run id to get path in wandb could"
            )
    ...
    def get_checkpoint_file(self, to_path=None, version=None, device=None):
        ...
        if not self.run_id:
            raise RuntimeError(
                "ExperimentWrappper:Error:Need to know run id to restore specific checkpoint from the could"
            )
        try:
            art_path = self._load_artifact(
                self.artifactname("checkpoint", version=version), to_path=to_path
            )
            ...
            wb.apis.CommError,
        ) as e:  # raised when file is corrupted or not found
            print(
                "ExperimentWrappper::Error::checkpoint from version '{}'is corrupted or lost: {}".format(
                    version if version else "latest", e
                )
            )
            raise e
    ...
    def get_best_model(self, to_path=None, device=None):
        ...
        if (
            "pre-trained" in self.in_config["experiment"]
            and self.in_config["experiment"]["pre-trained"] is not None
            and os.path.exists(self.in_config["experiment"]["pre-trained"])
        ):
            # local model available
            print(f"{self.__class__.__name__}::Info::Loading locally saved model")
            return self._load_model_from_file(
                self.in_config["experiment"]["pre-trained"], device
            )
        elif (
            "pre-trained" in self.in_config["NN"]
            and self.in_config["NN"]["pre-trained"] is not None
            and os.path.exists(self.in_config["NN"]["pre-trained"])
        ):
            # local model available
            print(f"{self.__class__.__name__}::Info::Loading locally saved model")
            return self._load_model_from_file(
                self.in_config["NN"]["pre-trained"], device
            )
        elif self.run_id:
            try:
                # best checkpoints have 'best' alias
                art_path = self._load_artifact(
                    self.artifactname("checkpoint", custom_alias="best"),
                    to_path=to_path,
                )
                ...
                raise RuntimeError(
                    "ExperimentWrappper:Error:No file with best weights found in run {}".format(
                        self.cloud_path()
                    )
                )
        else:
            raise RuntimeError(
                "ExperimentWrappper:Error:Need to know run_id to restore best model from the could OR path to the locally saved model "
            )
    ...
    def save_checkpoint(self, state, aliases=[], wait_for_upload=False):
        ...
        if not self.initialized:
            ...
            raise RuntimeError(
                "Experiment::cannot save checkpoint files to non-active wandb runs"
            )
        ...
        artifact = wb.Artifact(
            self.artifactname("checkpoint", with_version=False), type="checkpoint"
        )
        ...
        if wait_for_upload:
            self._wait_for_upload(
                self.artifactname("checkpoint", version=self.checkpoint_counter - 1)
            )
    ...
    def get_file(self, filename, to_path="."):
        ...
        if not self.run_id:
            raise RuntimeError(
                "ExperimentWrappper:Error:Need to know run id to restore a file from the could"
            )
        wb.restore(
            filename,
            run_path=self.project + "/" + self.run_id,
            replace=True,
            root=to_path,
        )
    ...
    def _wait_for_upload(self, artifact_name, max_attempts=10):
        ...
        if attempt > max_attempts:
            print(
                "Experiment::Warning::artifact {} is still not syncronized".format(
                    artifact_name
                )
            )
