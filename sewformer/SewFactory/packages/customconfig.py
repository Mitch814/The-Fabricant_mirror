...
):  # prevent from loading within Maya -- Py2.7 compatibility
    import platform
    import json
    import psutil
...
class Properties:
                ...
                if key in section["stats"]:
                    ...
                    if (
                        ...
                        if log_sum:
                            ...
                            updated = True
                        if log_avg:
                            section["stats"][key + "_avg"] = sum(stats_values) / len(
                                stats_values
                            )
                            if as_time:
                                section["stats"][key + "_avg"] = str(
                                    timedelta(seconds=section["stats"][key + "_avg"])
                                )
                            updated = True
        return updated
    ...
    def add_sys_info(self):
        ...
        if "win" in platform.system() or "Win" in platform.system():
            ...
            self.properties["system_info"]["GPU"] = [
                computer.Win32_VideoController()[i].name
    ...
    def stats_summary(self):
        ...
        if not (updated_frames and updated_render and updated_sim_time and updated_spf):
            print(
                "CustomConfig::Warning::Sim stats summary requested, but not all sections were updated"
            )
    ...
    def _recursive_dict_update(
        self, in_dict, new_dict, re_write=True, adding_tag="added", in_stats=False
    ):
        """
