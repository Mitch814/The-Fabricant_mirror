...
from SewFactory.packages import customconfig
...
def get_command_args():
    ...
    parser.add_argument(
        "--dataname", "-n", help="name of generated", default="deepfashion"
    )
    parser.add_argument(
        "--config",
        "-c",
        help="config file for dataset resource",
        default="meta_infos\\configs\\data_sim_configs.json",
    )
    parser.add_argument(
        "--sim-template",
        "-t",
        help="template props (predicted)",
        default="meta_infos\\sim_configs\\template.json",
    )
    parser.add_argument(
        "--predicted-root",
        "-r",
        help="path root to predicted results",
        default="other_scripts\\deepfahsion_results\\hats",
    )
    parser.add_argument(
        "--fbx-root",
        "-f",
        help="path root of posed fbxs (fbx or smpl info)",
        default="",
    )
    parser.add_argument(
        "--base-fbx",
        "-b",
        help="initial fbx model used to animate",
    ...
    parser.add_argument(
        "--attach-conf",
        "-a",
        help="config file for attachments to the human body",
        default="meta_infos\\configs\\attach_configs_predicted.json",
    )
    parser.add_argument(
        "--pose-root",
        "-p",
        help="root to estimated smpl pose files for deep fashion.",
        default="D:\\liulj_dev\\deepfashion\\spin_smpl_np",
    )
    parser.add_argument(
        "--output",
        "-o",
        help="save path for simulated dataset (NONE means save in resouce file)",
...
def init_mayapy():
    try:
        ...
        cmds.loadPlugin(
            "mtoa.mll"
        )  # https://stackoverflow.com/questions/50422566/how-to-register-arnold-render
        cmds.loadPlugin(
            "objExport.mll"
        )  # same as in https://forums.autodesk.com/t5/maya-programming/invalid-file-type-specified-atomimport/td-p/9121166
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.config["fbxs_root"]):
        fbxs = [
            os.path.join(args.config["fbxs_root"], fn)
            for fn in os.listdir(args.config["fbxs_root"])
    ...
    logging.basicConfig(
        filename=os.path.join(
            os.getcwd(), r"playblast_{}_run.txt".format(args.dataname)
        ),
        filemode="w",
        level=logging.INFO,
    ...
    playblast.simulate_predictions(
        predictions, retrieve_props=args.dataname == "sewfactory", logger=logger
    )
